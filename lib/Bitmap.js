...
Bitmap.prototype.loadPixelData = function() {
  console.log('loading pixel data: ', this.header);
  let offset = this.header.startOfPx;
  let row = [];
  let filePixels = [];
  let pixelFileData = '';
  for (var p = 0; p < this.header.pixelArraySize; p+=4) {
    // console.log(p)
    ...
    // this.pixelData = this.pixelData.concat(pixel);
    // this.pixelData.push(' - ');
    // if (p % this.header.pixelDataRowSize === 0) {
    //   this.pixelData.push('\n');
    // }
    offset += 4;
  }
  console.log('pixel data l: ',this.pixelData.length);
  fs.writeFileSync('./lib/pixelData', JSON.stringify(this.pixelData.join(' '), null, 2));
};
...
Bitmap.prototype.applyPixelData = function() {
  ...
  for (let p = 0; p < this.header.pixelArraySize; p+=4) {
    ...
    let b, g, r;
    // this.bmpBuf.writeUInt8(rand.rByteDec(), offset);
    // this.bmpBuf.writeUInt8(rand.rByteDec(), offset + 1);
    // this.bmpBuf.writeUInt8(rand.rByteDec(), offset + 2);
    ...
    // console.log(`p=${p}, pos=${xPos} ${yPos}, rgb=${b} ${g} ${r}`);
